...
describe('Garden native Workspace file adapter', () => {
  ...
  beforeEach(() => {
    ...
    db.findUsers.mockImplementation(async ({ openidId }) => [
      openidId === 'employee-a' ? userA : userB,
    ]);
    db.getFiles.mockResolvedValue([
      {
        ...
        filepath: '/uploads/u1/report.docx',
    ...
    const skill = {
      ...
      name: 'docx',
      version: 3,
      body: 'Use scripts/build.py',
    };
    getSkillDbMethods.mockReturnValue({
      getSkillById: jest.fn(async (id) => (id === '0123456789abcdef01234567' ? skill : null)),
      listSkillFiles: jest.fn(async () => [
        { relativePath: 'scripts/build.py', bytes: binary.length },
      ]),
      getSkillFileByPath: jest.fn(async (_id, name) =>
        name === 'scripts/build.py'
          ? {
              relativePath: name,
              bytes: binary.length,
              source: FileSources.local,
              filepath: '/uploads/u1/build.py',
            }
          : null,
      ),
    });
    getSkillToolDeps.mockReturnValue({ getStrategyFunctions });
    findAccessibleResources.mockImplementation(async ({ userId }) =>
      userId === 'u1' ? ['0123456789abcdef01234567'] : [],
    );
  });
  afterEach(() => {
    delete process.env.GARDEN_WORKSPACE_MCP_SERVICE_SECRET;
  });

  it('imports the existing PermissionService resource lookup at the real module boundary', () => {
    expect(
      typeof jest.requireActual('~/server/services/PermissionService').findAccessibleResources,
    ).toBe('function');
  });

  it('streams a raw 150 KB owned attachment and denies another user and missing service auth', async () => {
    const ok = await request(app)
      .get('/api/garden/workspace/attachments/file-1')
      .set(auth())
      .expect(200);
  ...
  it('returns native Skill relative assets only within current user access', async () => {
    const skillId = '0123456789abcdef01234567';
    const manifest = await request(app)
      .get(`/api/garden/workspace/skills/docx?skill_id=${skillId}`)
      .set(auth())
      .expect(200);
    ...
    const file = await request(app)
      .get(`/api/garden/workspace/skills/docx/files/scripts/build.py?version=3&skill_id=${skillId}`)
      .set(auth())
      .expect(200);
    ...
    await request(app)
      ...
      .expect(404);
    await request(app)
      .get(`/api/garden/workspace/skills/docx/files/scripts/build.py?version=2&skill_id=${skillId}`)
      .set(auth())
      .expect(404);
    await request(app)
      .get('/api/garden/workspace/skills/docx?skill_id=000000000000000000000000')
      .set(auth())
      .expect(404);
  });
  ...
  it('records a chosen binary result through native local storage and download path', async () => {
    ...
    await request(app)
      .post('/api/garden/workspace/outputs?filename=..%2Fforeign')
      .set(auth())
      .set('Content-Type', 'application/octet-stream')
      .send(binary)
      .expect(400);
